...
<div class="baberrage-stage" v-show="isShow" ref="stage">
  <div class="baberrage-top">
    <div  v-for="item in topQueue" v-bind:style="item.style" class="baberrage-item" v-bind:class="item.barrageStyle">
      <div class="baberrage-avatar"><img :src="item.avatar"></div>
      <div class="baberrage-msg">{{ item.msg }}</div>
    </div>
  </div>

  <div v-for="item in barrageList" v-bind:style="item.style" class="baberrage-item" v-bind:class="item.barrageStyle">
    <div class="baberrage-avatar"><img :src="item.avatar"></div>
    <div class="baberrage-msg">{{ item.msg }}</div>
  </div>

  <div class="baberrage-bottom">
    <div v-for="item in bottomQueue" v-bind:style="item.style" class="baberrage-item" v-bind:class="item.barrageStyle">
      <div class="baberrage-avatar"><img :src="item.avatar"></div>
      <div class="baberrage-msg">{{ item.msg }}</div>
    </div>
  </div>
...
export default {
  ...
  mounted () {
    ...
    window.cancelAnimationFrame = window.cancelAnimationFrame ||
      window.mozCancelAnimationFrame || function (requestID) {
  ...
  watch: {
    barrageList: function (newBarrageList) {
  ...
  methods: {
    ...
    queueRefresh (currentTime) {
      ...
      for (var i in this.bottomQueue) {
        item = this.bottomQueue[i]
        if (item.startTime + item.time * 1000 <= currentTime) {
